...
### Function to fit S-Learner BART with DiD regression in the leaves
  fit.bart.did <- function(df_train,df_test,features,draws=500,num_trees=50,max_depth=20)
  {
    ...
    ## Set t to (0,1)
    df_train$t <- df_train$t - min(df_train$t)
    df_test$t <- df_test$t - min(df_test$t)
...
time <- system.time({
  ...
  bart_placebo <- fit.bart.did(df_placebo,df_main,draws=draws,4:5)
  ## Obtain \hat{\tau} for main regression:
  bart_main <- fit.bart.did(df_main,df_main,draws=draws,4:5)
})
```
...
time <- system.time(
  {
    test <- mapply(function(i,j) searchPTA(subset(df_main,select=4:5),bart_placebo$b1[,i],bart_placebo$b0[,i],bart_main$b1[,j],bart_main$b0[,j],0.01),
               # i=sample(1:draws,replace=TRUE),j=1:draws,SIMPLIFY = FALSE)
               i=1:draws,j=1:draws,SIMPLIFY = FALSE)
  }
)

summary(sapply(test, function(i) length(i$catt)))
```

```{r}
catt2 <- unlist(lapply(1:length(test), function(i) test[[i]]$catt[x[t>=0]==2]))
hist(catt2)
abline(v=tau_1[2]+alpha_1[2],lty=2)
###
catt3 <- unlist(lapply(1:length(test), function(i) test[[i]]$catt[x[t>=0]==3]))
hist(catt3)
abline(v=0.5*(tau_1[3]+alpha_1[3]+tau_1[4]+alpha_1[4]),lty=2)
###
catt4 <- unlist(lapply(1:length(test), function(i) test[[i]]$catt[x[t>=0]==4]))
hist(catt4)
abline(v=0.5*(tau_1[3]+alpha_1[3]+tau_1[4]+alpha_1[4]),lty=2)
```
```{r}
test <- mapply(function(i,j) searchPTA(subset(df_main,select=4:5),bart_placebo$b1[,i],bart_placebo$b0[,i],bart_main$b1[,j],bart_main$b0[,j],0.05),
               # i=sample(1:draws,replace=TRUE),j=1:draws,SIMPLIFY = FALSE)
               i=1:draws,j=1:draws,SIMPLIFY = FALSE)
```
...
```{r}
test <- mapply(function(i,j) searchPTA(subset(df_main,select=4:5),bart_placebo$b1[,i],bart_placebo$b0[,i],bart_main$b1[,j],bart_main$b0[,j],0.1),
               # i=sample(1:draws,replace=TRUE),j=1:draws,SIMPLIFY = FALSE)
               i=1:draws,j=1:draws,SIMPLIFY = FALSE)
```
...
```{r}
test <- mapply(function(i,j) searchPTA(subset(df_main,select=4:5),bart_placebo$b1[,i],bart_placebo$b0[,i],bart_main$b1[,j],bart_main$b0[,j],0.5),
               # i=sample(1:draws,replace=TRUE),j=1:draws,SIMPLIFY = FALSE)
               i=1:draws,j=1:draws,SIMPLIFY = FALSE)
```
